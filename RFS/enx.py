...
def create_enx_resnet18(num_classes=10, pretrained=False, **kwargs):
    """
    ...
    model = timm.create_model('legacy_seresnet18', pretrained=False, num_classes=num_classes, **kwargs)

    # ENX 自定义参数，根据您的示例设置
    channels = [512, 256, 128, 64]
    iters = [3, 4, 5, 6]
    gammas = [0.99, 0.98, 0.95, 0.9] # 与 channels 对应：512->3 iter, 64->6 iter

    # 遍历模型中的所有模块，寻找并替换 SEModule
    replacement_count = 0
    for name, module in model.named_modules():
        # 在 timm 的 legacy_seresnet 中，SEModule 被命名为 'se'
        # 且其类型是 senet.SEModule
        # 由于我们无法直接导入 senet.SEModule，我们通过检查名称和结构来识别它
        if isinstance(module, timm.models.senet.SEModule):
            # 提取通道数 (SEModule 的输入通道数等于 fc1 的输入通道数)
            cin = module.fc1.in_channels 
            # 查找匹配的自定义 ENX 参数
            try: 
                index = channels.index(cin)  
                enx_iters = iters[index]
                enx_gamma = gammas[index]
                ...
                # 通道数不匹配，可能是模型结构中的其他模块，跳过
                continue
                
    print(f"--- 替换完成：共替换 {replacement_count} 个 SE 模块为 ENX 模块。---")
    return model
...
if __name__ == '__main__':
    ...
    enx_model = create_enx_resnet18(num_classes=100, pretrained=False)
