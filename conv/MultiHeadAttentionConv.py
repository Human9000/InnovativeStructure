...
class MultiHeadAttentionConv(nn.Module):
    def __init__(self,
        ...
        self.out_proj = conv(out_channels, out_channels, 1, bias=bias)
    ...
    def forward(self, x):
        ...
        scores = torch.matmul(query, key.transpose(-1, -2)) / math.sqrt(self.num_heads)  # (b, h, 1, d)
        ...
        attn_output = self.out_proj(attn_output.reshape(*value.shape)) + value  # (b, h*d, *l)
        return attn_output, attn_weights


class MultiHeadAttentionConv1d(MultiHeadAttentionConv):
...
class MultiHeadAttentionConv2d(MultiHeadAttentionConv):
    def __init__(self, num_heads, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False):
        super().__init__(num_heads, in_channels, out_channels, kernel_size, stride, padding, bias, conv=nn.Conv2d)


class MultiHeadAttentionConv3d(MultiHeadAttentionConv):
    def __init__(self, num_heads, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False):
